...
async def start_registration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_send_message(
        ...
        f"🗓️ <b>Дата:</b> {escape_html(SCHEDULE['next_meeting'])}\n"
...
async def get_letter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"\n📝 get_letter: user_id={user_id}, ответ='{text}'")
    if user_id not in user_sessions:
        await update.message.reply_text("⚠️ Сессия устарела. /start")
        return ConversationHandler.END
    if text.startswith('/'):
...
async def get_letter_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    text = update.message.text.strip()
    print(f"\n📄 get_letter_info: user_id={user_id}")
    if user_id not in user_sessions:
        await update.message.reply_text("⚠️ Сессия устарела. /start")
        return ConversationHandler.END
    if text.startswith('/'):
...
async def handle_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_yes":
        ...
        if success:
            ...
            await safe_edit_message(
                query,
                f"❌ <b>ОШИБКА РЕГИСТРАЦИИ</b>\n\nСвяжитесь с организатором: {escape_html(ORGANIZERS)}"
            )
        if user_id in user_sessions:
            del user_sessions[user_id]
        return ConversationHandler.END
...
async def show_letter_requests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for i, req in enumerate(requests, 1):
            report += f"👤 *{i}. {req.get('ФИО участника', 'Нет')}*\n"
            report += f"   👨‍💼 Директор: {req.get('ФИО директора/ректора', 'Нет')}\n"
            report += f"   🏛️ Вуз: {req.get('Полное наименование учебного заведения', 'Нет')}\n"
            report += f"   📖 Курс: {req.get('Класс/курс (направление)', 'Нет')}\n"
            report += f"   📝 Мероприятие: {req.get('Наименование мероприятия', 'Нет')}\n"
            report += f"   📄 Вид: {req.get('Вид письма', 'Нет')}\n"
            report += f"   📅 Дата: {req.get('Дата заявки', 'Нет')}\n\n"
        await update.message.reply_text(report, parse_mode='Markdown')
    except Exception as e:
        await update.message.reply_text(f"❌ {e}")

async def send_rating_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправить опрос с оценкой мероприятия (только для админа)"""
    user_id = update.effective_user.id
    ...
    try:
        ...
        # ВАРИАНТ 1: Кнопки в две строки (всегда видны полностью)
        keyboard = [
...
async def rating_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if recent:
            report += "*Последние:*\n"
            for r in recent:
                report += f"   • {r['name']} - {r['rating']}⭐ ({r['date']})\n"
        await update.message.reply_text(report, parse_mode='Markdown')
    except Exception as e:
        await update.message.reply_text(f"❌ {e}")
